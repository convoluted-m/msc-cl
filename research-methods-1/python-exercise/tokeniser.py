# # Implement a tokeniser and a function that returns basic corpus statistics
# class Tokeniser:
#     '''A tokeniser class'''
#     def __init__(self):
#         self.token_dict = {} # dictionary to store token counts
#         self.is_trained = False # to check if the tokeniser has been trained
...
#             else:
#                 self.token_dict[i] += 1
#         self.is_trained = True # set attribute to True to check in the function below
...
def get_stats(text):
    ...
    type_count = len(token_dict) # returns number of keys which are unique tokens
