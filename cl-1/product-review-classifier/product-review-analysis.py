...
## FIT a logistic regression model for classification with one hidden layer
...
weights_input_hidden = np.random.rand(num_features, num_hidden_neurons)
bias_hidden = np.random.rand(num_hidden_neurons)
weights_hidden_output = np.random.rand(num_hidden_neurons)
...
for i in range(n_iters):
    for batch in batches:
        ...
        # Forward pass: calculate hidden layer activations
        z_hidden = batch_train_matrix.dot(weights_input_hidden) + bias_hidden
        a_hidden = 1 / (1 + np.exp(-z_hidden))  # Sigmoid activation function

        # Forward pass: calculate output layer activations
        z_output = a_hidden.dot(weights_hidden_output) + bias_output
        q = 1 / (1 + np.exp(-z_output))  # Sigmoid activation function
        ...
        dz_output = q - batch_y_train
        dw_hidden_output = a_hidden.T.dot(dz_output) / len(batch_y_train)
        db_output = np.sum(dz_output) / len(batch_y_train)

        # Calculate gradients for hidden layer
        da_hidden = dz_output.reshape(-1, 1).dot(weights_hidden_output.reshape(1, -1))
        dz_hidden = da_hidden * a_hidden * (1 - a_hidden)  # Derivative of sigmoid
        dw_input_hidden = batch_train_matrix.T.dot(dz_hidden) / len(batch_y_train)
        db_hidden = np.sum(dz_hidden, axis=0) / len(batch_y_train)

        # Update parameters
        weights_hidden_output -= lr * dw_hidden_output
        bias_output -= lr * db_output
        weights_input_hidden -= lr * dw_input_hidden
        bias_hidden -= lr * db_hidden
...
## DEV set testing
z_hidden_dev = dev_matrix.dot(weights_input_hidden) + bias_hidden
a_hidden_dev = 1 / (1 + np.exp(-z_hidden_dev))
z_output_dev = a_hidden_dev.dot(weights_hidden_output) + bias_output
q_dev = 1 / (1 + np.exp(-z_output_dev))
y_dev_pred = []
